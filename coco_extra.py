...
#
def save_annotations_and_imgs(coco, dataset, filename, objs, des_img_path, anno_path):
    # eg:COCO_train2014_000000196610.jpg-->COCO_train2014_000000196610.xml
    anno_path = os.path.join(anno_path, filename[:-3] + 'xml')
    # print(anno_path)
    img_path=os.path.join(dataDir, dataset ,filename)
    #img_path = dataset + '//' + filename
    # print(img_path)
    dst_imgpath = os.path.join(des_img_path, filename)
    # print(dst_imgpath)
...
def generate(img_path, anno_path):
    for dataset in datasets_list:
        ...
        for cls in classes_names:
            ...
            for imgId in tqdm(img_ids):
                ...
                # print(objs)
                save_annotations_and_imgs(coco, dataset, filename, objs, img_path, anno_path)
...
if __name__ == "__main__":
    ...
    dataDir = 'E:/annotations_trainval2017/'
    # 转化后的文件保存路径
    train_savepath = "E:/annotations_trainval2017/wanted/"
    # 转化后图片保存路径
    train_img_dir = os.path.join(train_savepath , 'JPEGImages', 'train')
    # 转化后xml保存路径
    train_anno_dir = os.path.join(train_savepath, 'Annotations', 'train')
    # 选用的数据集列表
    ...
    # 生成voc格式的数据集
    generate(train_img_dir, train_anno_dir)

    # 转化后图片保存路径
    val_img_dir = os.path.join(train_savepath, 'JPEGImages', 'val')
    # 转化后xml保存路径
    val_anno_dir = os.path.join(train_savepath , 'Annotations', 'val')
    # 选用的数据集列表
    # datasets_list=['train2014', 'val2014']
    ...
    mkr(val_anno_dir)
    # 生成voc格式的数据集
    generate(val_img_dir, val_anno_dir)
